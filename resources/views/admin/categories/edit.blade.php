...
@section('content')
    <div class="card max-w-2xl mx-auto">
        <div class="card-header border-b border-slate-100 p-6 flex justify-between items-center">
            <h3 class="text-lg font-bold text-slate-800">Edit Category: {{ $category->name }}</h3>
        ...
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex justify-end">
                <button type="submit"
                    class="bg-slate-800 text-white px-4 py-2 rounded-md hover:bg-slate-700 transition-colors">Update
                    Category</button>
            </div>
        </form>
    </div>
