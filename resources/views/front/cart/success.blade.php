@extends(request()->ajax() ? 'layouts.ajax' : 'layouts.app')

@section('content')
    <div class="container py-5 text-center">
        <div class="card p-5 border-0 shadow-sm mx-auto" style="max-width: 600px;">
            <div class="mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" fill="#28a745"
                    class="bi bi-check-circle-fill" viewBox="0 0 16 16">
                    <path
                        d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z" />
                </svg>
            </div>
            <h1 class="display-4 fw-bold text-success mb-3">Order Placed!</h1>
            <p class="lead text-muted mb-4">Thank you for shopping with Wooden Oak Studio. Your order has been successfully
                placed and will be delivered shortly.</p>
            <a href="{{ route('home') }}" class="btn btn-primary btn-lg px-5">Back to Home</a>
        </div>
    </div>
@endsection